...
**Concurrency & availability.** Verdict acceptance on a hold is atomic (single-writer / optimistic locking), so two operators acting at once can't double-count and one operator can't cast both verdicts; claiming (FR-3) reduces contention upstream. Because two *distinct* sign-offs are structurally required, a hold cannot clear with only one operator present — so the **availability policy** must be defined: park indefinitely (safe default), or escalate to a nominated third eligible signatory after a timeout for teams needing throughput. Escalations and timeouts are themselves audit events.
